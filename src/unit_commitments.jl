...
function stochastic_uc(scenarios, scenario_weights, gen_df, network, mip_gap)
    UC = Model(HiGHS.Optimizer)
    set_optimizer_attribute(UC, "mip_rel_gap", mip_gap) 

    @assert length(scenarios) == length(scenario_weights) "Length mismatch"
    Ns = length(scenarios)
    Sc = 1:Ns
    demand_s        = [scenarios[s][:loads]          for s in Sc] # Matrix T x Z
    variability_s   = [scenarios[s][:gen_var]        for s in Sc] # Matrix T x G
    sample_weight_s = [scenarios[s][:sample_weight]  for s in Sc] # Vector length T
    ...
    # Decision variables   
    @variables(UC, begin
        GEN[Sc, G, T]  >= 0     # generation
        GENAUX[Sc, G_thermal, T] >= 0 # auxiliary generation variable
        COMMIT[Sc, G_thermal, T], Bin # commitment status (Bin=binary)
        START[Sc, G_thermal, T], Bin  # startup decision
        SHUT[Sc, G_thermal, T], Bin   # shutdown decision
        RESUP[Sc, G_thermal, T]       # up reserve capacity
        RESDN[Sc, G_thermal, T]       # down reserve capacity
        FLOW[Sc, L, T] # power flows

        # battery vars
        0 <= SOC[Sc, B, T] <= storage_df[storage_df.r_id .== b, :existing_cap_mw][1]  # state of charge
        0 <= CHARGE[Sc, B, T] <= storage_df[storage_df.r_id .== b, :ramp_up_percentage][1] * 
                                    storage_df[storage_df.r_id .== b, :existing_cap_mw][1]
        0 <= DISCHARGE[Sc, B, T] <= storage_df[storage_df.r_id .== b, :ramp_dn_percentage][1] *
                                    storage_df[storage_df.r_id .== b, :existing_cap_mw][1]  
    end)
                
    # Objective function
    # Expected operational costs -- *Weighted* for stochastic optimization
    # Create separate expressions for each scenario
    eScenarioCosts = [@expression(UC,
        sum( (gen_df[gen_df.r_id .== i,:heat_rate_mmbtu_per_mwh][1] * gen_df[gen_df.r_id .== i,:fuel_cost][1] +
            gen_df[gen_df.r_id .== i,:var_om_cost_per_mwh][1]) * GEN[s,i,t] 
                        for i in G_nonvar for t in T) + 
        sum(gen_df[gen_df.r_id .== i,:var_om_cost_per_mwh][1] * GEN[s,i,t] 
                        for i in G_var for t in T) + 
        sum(gen_df[gen_df.r_id .== i,:start_cost_per_mw][1] * 
            gen_df[gen_df.r_id .== i,:existing_cap_mw][1] *
            START[s, i, t] 
    ...
    @expression(UC, eCosts,
        sum(scenario_weights[s] * eScenarioCosts[s] for s in Sc)
    )

    @objective(UC, Min, eCosts)
    
    # OPF constraints

    # Auxiliary variable: total generation per zone and time
    @variable(UC, ZONE_GEN[s in Sc, z in Z, t in T] >= 0)

    @constraint(UC, ZoneGenDef[s in Sc, z in Z, t in T],
        ZONE_GEN[s, z, t] == sum(GEN[s, i, t] for i in G if gen_df[gen_df.r_id .== i, :zone][1] == z)
    )

    # power line capacities
    @constraint(UC, FlowMax[s in Sc, l in L, t in T],
        FLOW[s, l, t] <= network.line_max_flow_mw[l]
    )

    @constraint(UC, FlowMin[s in Sc, l in L, t in T],
        FLOW[s, l, t] >= -network.line_max_flow_mw[l]
    )

    # demand balance by zone -----------------------------------------------
    @constraint(UC, NodalBalance[s in Sc, z in Z, t in T],
        ZONE_GEN[s, z, t] + 
        sum(DISCHARGE[s, b, t] - CHARGE[s, b, t] for b in B if storage_df[storage_df.r_id .== b, :zone][1] == z) +
        sum(FLOW[s, l, t] * network[l, "$z"] * (1 - network.line_loss[l]/2) for l in L if network[l, "$z"] != 0) ==
        demand_s[s][(demand_s[s][:hour] .== t) .& (demand_s[s][:zone] .== z), :demand][1]
    )

    # Capacity constraints (thermal generators requiring commitment)
    @constraint(UC, Cap_thermal_min[s in Sc, i in G_thermal, t in T], 
        GEN[s, i, t] >= COMMIT[s, i, t] * gen_df[gen_df.r_id .== i,:existing_cap_mw][1] *
                        gen_df[gen_df.r_id .== i,:min_power][1])
    @constraint(UC, Cap_thermal_max[s in Sc, i in G_thermal, t in T], 
        GEN[s, i, t] <= COMMIT[s, i, t] * gen_df[gen_df.r_id .== i,:existing_cap_mw][1])

    # Capacity constraints (non-variable generation not requiring commitment)
    @constraint(UC, Cap_nt_nonvar[s in Sc, i in G_nt_nonvar, t in T], 
        GEN[s, i, t] <= gen_df[gen_df.r_id .== i,:existing_cap_mw][1])

    # Capacity constraints (variable generation) 0----------------------------

    @constraint(UC, Cap_var[s in Sc, i in G_var, t in T], 
        GEN[s, i, t] <= variability_s[s][(variability_s[s][:hour] .== t) .& (variability_s[s][:r_id .== i]), :cf][1] *
                    gen_df[gen_df.r_id .== i, :existing_cap_mw][1])
    
    # battery constraints 
    @constraint(UC, cStateOfChargeEnd[s in Sc, b in B],
        SOC[s, b, T[end]] == 0.5 * storage_df[storage_df.r_id .== b, :existing_cap_mw][1]
    )
    @constraint(UC, cStateOfChargeStart[s in Sc, b in B],
        SOC[s, b, T[1]] == 0.5 * storage_df[storage_df.r_id .== b, :existing_cap_mw][1] + # start charge
                (CHARGE[s, b, T[1]] * storage_df[storage_df.r_id .== b, :charge_efficiency][1] -
                DISCHARGE[s, b, T[1]] / storage_df[storage_df.r_id .== b, :discharge_efficiency][1])
    )
    @constraint(UC, cStateOfCharge[s in Sc, b in B, t in T[2:end]],
        SOC[s, b, t] == SOC[s, b, t-1] +
                    (CHARGE[s, b, t] * storage_df[storage_df.r_id .== b, :charge_efficiency][1] -
                    DISCHARGE[s, b, t] / storage_df[storage_df.r_id .== b, :discharge_efficiency][1])
    )

    # Unit commitment constraints
    @constraint(UC, Startup[s in Sc, i in G_thermal, t in T],
        COMMIT[s, i, t] >= sum(START[s, i, tt] 
                        for tt in intersect(T,
                            (t-gen_df[gen_df.r_id .== i,:up_time][1]):t)))

    @constraint(UC, Shutdown[s in Sc, i in G_thermal, t in T],
        1-COMMIT[s, i, t] >= sum(SHUT[s, i, tt] 
                        for tt in intersect(T,
                            (t-gen_df[gen_df.r_id .== i,:down_time][1]):t)))
    
    @constraint(UC, CommitmentStatus[s in Sc, i in G_thermal, t in T_red],
        COMMIT[s, i, t+1] - COMMIT[s, i, t] == START[s, i, t+1] - SHUT[s, i, t+1]
    )
    # New auxiliary variable GENAUX for generation above the minimum output level
    # for committed thermal units (only created for thermal generators)
    @constraint(UC, AuxGen[s in Sc, i in G_thermal, t in T],
        GENAUX[s, i, t] == GEN[s, i, t] - COMMIT[s, i, t] * gen_df[gen_df.r_id .== i,:existing_cap_mw][1] *
                        gen_df[gen_df.r_id .== i,:min_power][1])
    
    # Ramp equations for thermal generators (constraining GENAUX)
    @constraint(UC, RampUp_thermal[s in Sc, i in G_thermal, t in T_red], 
        GENAUX[s, i, t+1] - GENAUX[s, i, t] <= gen_df[gen_df.r_id .== i,:existing_cap_mw][1] * 
                                 gen_df[gen_df.r_id .== i,:ramp_up_percentage][1] )

    @constraint(UC, RampDn_thermal[s in Sc, i in G_thermal, t in T_red], 
        GENAUX[s, i, t] - GENAUX[s, i, t+1] <= gen_df[gen_df.r_id .== i,:existing_cap_mw][1] * 
                                 gen_df[gen_df.r_id .== i,:ramp_dn_percentage][1] )

    # Ramp equations for non-thermal generators (constraining total generation GEN)
    @constraint(UC, RampUp_nonthermal[s in Sc, i in G_nonthermal, t in T_red], 
        GEN[s, i, t+1] - GEN[s, i, t] <= gen_df[gen_df.r_id .== i,:existing_cap_mw][1] * 
                                 gen_df[gen_df.r_id .== i,:ramp_up_percentage][1] )

    @constraint(UC, RampDn[s in Sc, i in G_nonthermal, t in T_red], 
        GEN[s, i, t] - GEN[s, i, t+1] <= gen_df[gen_df.r_id .== i,:existing_cap_mw][1] * 
                                 gen_df[gen_df.r_id .== i,:ramp_dn_percentage][1] )
    
                                 
    # Reserve equations
    # (1) Reserves limited by committed capacity of generator
    @constraint(UC, ResUpCap[s in Sc, i in G_thermal, t in T],
        RESUP[s, i, t] <= COMMIT[s, i, t] * gen_df[gen_df.r_id .== i,:existing_cap_mw][1]
                        - GEN[s, i, t])

    @constraint(UC, ResDnCap[s in Sc, i in G_thermal, t in T],
        RESDN[s, i, t] <= GEN[s, i, t] - 
                        COMMIT[s, i, t] * gen_df[gen_df.r_id .== i,:existing_cap_mw][1] *
                        gen_df[gen_df.r_id .== i,:min_power][1])

    # (2) Reserves limited by ramp rates
    @constraint(UC, ResUpRamp[s in Sc, i in G_thermal, t in T],
        RESUP[s, i, t] <= gen_df[gen_df.r_id .== i,:existing_cap_mw][1] * 
                          gen_df[gen_df.r_id .== i,:ramp_up_percentage][1])

    @constraint(UC, ResDnRamp[s in Sc, i in G_thermal, t in T],
        RESDN[s, i, t] <= gen_df[gen_df.r_id .== i,:existing_cap_mw][1] * 
                          gen_df[gen_df.r_id .== i,:ramp_dn_percentage][1])

    # Compute reserve requirements
    ResReqUp = Dict(T[i] => 300  
              + 0.05 * demand_s[s][demand_s[s][:hour] .== T[i],:demand][1]
                    for i in 1:length(T), s in Sc)
    ...
    # (3) Overall reserve requirements
    @constraint(UC, ResUpRequirement[t in T],
        sum(RESUP[s, i, t] for i in G_thermal, s in Sc) >=
                ResReqUp[t])

    @constraint(UC, ResDnRequirement[t in T],
        sum(RESDN[s, i, t] for i in G_thermal, s in Sc) >=
                ResReqDn[t])
